...
describe("Tests for different cmd calls", () => {
	it("Should write the output to the passed destination folder", () => {
		...
		const command = `node node_modules/jsdoc/jsdoc -r exampleProject/src -t src-out/core -d ${targetFolder}`;
		execSync(command);
	...
	it("Should write the output to the passed destination folder with the passed file name", () => {
		...
		const command = `node node_modules/jsdoc/jsdoc -r exampleProject/src -t src-out/core -d ${targetFilePath}`;
		execSync(command);
